...
{
	VectorString weights(m_outputEdge.size());
	VectorEdge edge(m_outputEdge.size());
	std::ofstream ofs(OUTPUT_GRAPH_NAME);

	for (size_t i = 0, x = 1, index = 0; i < m_outputEdge.size(); ++i, ++index)
	{
		...
		edge[i] = { index, m_outputEdge[i].first };
	}
...
{
	m_copyEdge.resize(m_inputEdge.size());

	std::copy(m_inputEdge.begin(), m_inputEdge.end(), m_copyEdge.begin());
	std::sort(m_copyEdge.begin(), m_copyEdge.end());
	m_copyEdge.erase(std::unique(m_copyEdge.begin(), m_copyEdge.end()), m_copyEdge.end());

	m_outputEdge.resize(m_inputSize * m_copyEdge.size());
	...
	for (size_t i = 0; i < m_copyEdge.size(); ++i)
	{
		...
		{
			auto it = std::find(m_copyEdge.begin(), m_copyEdge.end(), m_inputEdge[index]);
			int indexFindCopy = int(std::distance(m_copyEdge.begin(), it));
			m_outputEdge[indexEdge] = { indexFindCopy, m_copyEdge[i].second };
...
void CAutomatMealy::PrintInfo() const
{
	...
	for (size_t i = 0, x = 0; i < m_outputEdge.size(); ++i)
	{
		...
		m_output << SYMBOL_Z << m_outputEdge[i].first << TAB;
	}
}